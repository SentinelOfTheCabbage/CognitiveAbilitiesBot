...
class DbTeller:
    ...
    def __init__(self):
        self.connect = None
        self.host = 'ec2-46-137-84-173.eu-west-1.compute.amazonaws.com'
        self.database = 'dbrmibj8vg0k2t'
        self.user = 'hcboutmgqhkbuc'
        self.port = '5432'
        self.password = 'ce9926f95e8ef77dac6dbaddd0bf20a88dd4e4b672566f08987633e684bdeed0'
    ...
    def get_products_list(self) -> dict:
        ...
        for pos, key in enumerate(keys):
            ...
            if ('prod' in key) or ('fish' in key):
                ...
                if 'mask' not in key:
                    price_desk = f'{int(prices[pos])}р/{int(1000*pack_sizes[pos])}г'
                else:
    ...
    def give_revard_by_promo(self, bonus_code: str):
        ...
        query = f"""UPDATE bonuses
            SET bonuses_amount = bonuses_amount + {revard_size}
            WHERE bonus_code = '{bonus_code}'
        """
        return self.execute(query)
