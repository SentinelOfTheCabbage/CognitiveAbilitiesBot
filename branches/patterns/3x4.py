...
def svg_to_png(filename='pattern.svg'):
    ...
    renderPM.drawToFile(drawing, '3x4.png', fmt='PNG')
...
{}
    ...
    '''
    rect_pattern = '\t<rect x="{}" y = "{}" width="88" height="88" rx="15" fill="#123456"/>'
    rectangles = []
    for i in range(4):
        for j in range(3):
            rect = rect_pattern.format(
                34 + 122 * j, 100 + 12 + 96 * i)
            rectangles.append(rect)
    result = file_pattern.format('\n'.join(rectangles))
    with open('3x4.svg', 'w', encoding='cp1251') as file:
        file.write(result)

    svg_to_png(filename='3x4.svg')
