...
IMPORTS = set(['re', 'Phase', 'types', 'datetime',
               'timedelta', 'MKAD', 'Point', 'Polygon', 'send_adm_message', 'deepcopy', 'time', 'randint', 'randrange', 'shuffle', 'svg2rlg', 'svg2rlg', 'renderPM', 'getcwd'])
...
def get_triggers():
    triggers = []
    triggers.append('squares')
    return triggers


def get_restarts():
    restarts = []
    restarts.append('squares')
    return restarts
